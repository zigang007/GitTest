 ...
 * 当升级编辑器时，可直接使用旧版配置文件替换新版配置文仿,不用担心旧版配置文件中因缺少新功能所需的参数而导致脚本报错〿
 **************************提示********************************/
...
(function () {
     ...
     * 鉴于很多同学在使用编辑器的时候出现的种种路径问题，此处强烈建议大家使甿"相对于网站根目录的相对路徿"进行配置〿
     * "相对于网站根目录的相对路徿"也就是以斜杠开头的形如"/myProject/umeditor/"这样的路径〿
    ...
    window.UMEDITOR_CONFIG = {
        ...
        //为编辑器实例添加一个路径，这个不能被注釿
        UMEDITOR_HOME_URL : URL

        //图片上传配置匿
        ,imageUrl:URL+"php/imageUp.php"             //图片上传提交地址
        ,imagePath:URL + "php/"                     //图片修正地址，引用了fixedImagePath,如有特殊需求，可自行配罿
        ...
        //语言配置顿,默认是zh-cn。有需要的话也可以使用如下这样的方式来自动多语言切换，当然，前提条件是lang文件夹下存在对应的语言文件＿
        ...
        //主题配置顿,默认是default。有需要的话也可以使用如下这样的方式来自动多主题切换，当然，前提条件是themes文件夹下存在对应的主题文件：
        //现有如下皮肤:default
        //,theme:'default'
        ...
        //,initialContent:'欢迎使用UMEDITOR!'    //初始化编辑器的内宿,也可以通过textarea/script给值，看官网例孿
        ...
        //,textarea:'editorValue' // 提交表单时，服务器获取编辑器提交内容的所用的参数，多实例时可以给容器name属性，会将name给定的值最为每个实例的键值，不用每次实例化的时候都设置这个倿

        //,focus:false //初始化时，是否让编辑器获得焦点true或false

        //,autoClearEmptyNode : true //getContent时，是否删除空的inlineElement节点（包括嵌套的情况＿

        //,fullscreen : false //是否开启初始化时即全屏，默认关闿

        //,readonly : false //编辑器初始化结束吿,编辑区域是否是只读的，默认是false

        //,zIndex : 900     //编辑器层级的基数,默认昿900

        //如果自定义，最好给p标签如下的行高，要不输入中文时，会有跳动愿
        //注意这里添加的样式，最好放圿.edui-editor-body .edui-body-container这两个的下边，防止跟页面上css冲突
        //,initialStyle:'.edui-editor-body .edui-body-container p{line-height:1em}'

        //,autoSyncData:true //自动同步编辑器要提交的数捿

        //,emotionLocalization:false //是否开启表情本地化，默认关闭。若要开启请确保emotion文件夹下包含官网提供的images表情文件夿

        //,allHtmlEnabled:false //提交到后台的数据是否包含整个html字符丿
...
//          ]
        ...
        //paragraph
        //段落格式 值留空时支持多语言自动识别，若配置，则以配置值为凿
        ...
        //填写过滤规则
        ,filterRules: {}
        // xss 过滤是否开吿,inserthtml等操使
 		,xssFilterRules: true
 		//input xss过滤
 		,inputXssFilter: true
 		//output xss过滤
 		,outputXssFilter: true
 		// xss过滤白名卿 名单来源: https://raw.githubusercontent.com/leizongmin/js-xss/master/lib/default.js
        ,whiteList: {
